{{-- Nombramiento principal del trabajador  --}}
<div class="card shadow-sm animate__animated animate__fadeInDown">
    <div class="card-body">
        @if (!empty($Principal))
            <h5 class="card-title text-center">Nombramiento principal</h5>
            ...
            <div class="row ">
                <div class="col-12 col-md-6 col-xl-6 mt-3">
                    ...
                    <x-icon-cont
                        ...
                        label="Nombramiento" text="{{ $Principal->nombramientoPersona->nombre }}" id="P_nombra"
                        data-id_nombra="{{ $Principal->nombramiento }}" />
                    ...
                    <x-icon-cont
                        ...
                        label="Categoría" text="{{ $Principal->trabajoCategoria->nombre }}" id="cate_name" />

                    <span id="id_cate" class="d-none">{{ $Principal->id_categoria }} </span>

                    @if ($Principal->distincionAdicional)
                        <x-icon-cont
                            ...
                            label="Distinción adicional" text="{{ $Principal->distincionAdicional->nombre }}"
                            id="P_dist" />
                    @else
                        <x-icon-cont
                            icon='<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24"><path fill="#059669" d="M1 3a2 2 0 0 1 2-2h6.5a2 2 0 0 1 2 2v6.5a2 2 0 0 1-2 2H7v4.063C7 16.355 7.644 17 8.438 17H12.5v-2.5a2 2 0 0 1 2-2H21a2 2 0 0 1 2 2V21a2 2 0 0 1-2 2h-6.5a2 2 0 0 1-2-2v-2.5H8.437A2.94 2.94 0 0 1 5.5 15.562V11.5H3a2 2 0 0 1-2-2Zm2-.5a.5.5 0 0 0-.5.5v6.5a.5.5 0 0 0 .5.5h6.5a.5.5 0 0 0 .5-.5V3a.5.5 0 0 0-.5-.5ZM14.5 14a.5.5 0 0 0-.5.5V21a.5.5 0 0 0 .5.5H21a.5.5 0 0 0 .5-.5v-6.5a.5.5 0 0 0-.5-.5Z"/></svg>'
                    ...
                    <x-icon-cont
                        ...
                        label="Área de adscripción" text="{{ $Principal->area_distincion ?? '--' }}" id="P_area" />
                    {{-- $Principal->id_estado --}}
                    @php
                        $color = '';
                        switch ($Principal->estado->nombre) {
                            case 'Inactivo':
                                $color = '#dc2626';
                                break;
                    ...
                    <x-icon-cont
                        ...
                        label="Estatus laboral" text="{{ $Principal->estado->nombre }}" id="Person_Status" />
                    ...
                    <div class="row pt-2">
                        <div class="form-group col-6">
                            <x-icon-cont
                                ...
                                label="T. Contrato" text="{{ $Principal->tipo_contrato }} " id="P_contrato" />
                        ...
                        <div class="form-group col-6">
                            ...
                            @php
                                $fecha_termino = '';
                                if ($Principal->fecha_termino != null) {
                                    $fecha_termino = \Carbon\Carbon::parse($Principal->fecha_termino)
                                        ->locale('es')
                                        ->isoFormat('LL');
                                } else {
                                    $fecha_termino = '--';
                            ...
                            <span class="d-none" id="P_termino">{{ $Principal->fecha_termino }}</span>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="form-group col-6">
                            ...
                            <x-icon-cont
                                ...
                                label="H. de trabajo" text="{{ $Principal->horas_trabajo ?? '--' }}" id="P_horas" />
                        ...
                        <div class="form-group col-6">
                            ...
                            <x-icon-cont
                                ...
                                label="Turno" text="{{ $Principal->turno ?? '--' }}" id="P_turno" />
                        </div>
                    </div>

                    <div class="row">
                        @php
                            $horarios = explode(',', $Principal->horario_oficial);
                ...
                <div class="row mt-1">
                    <div class="form-group pt-2 col-12">
                        ...
                        <div class="row">
                            ...
                            @if (empty($Principal->departamento))
                                <ul class="list-group shadow-sm p-1 bg-light rounded-3 mb-2">
                                    @foreach ($Principal->departamento as $departamento)
                                        <li
                                            class="list-group-item border-0 d-flex align-items-center gap-1 bg-transparent ">
                            ...
                            @else
                                <div class="">
                                    @if ($Principal->departamento->isNotEmpty())
                                        <ul class="list-group shadow-sm p-1 bg-light rounded-3 mb-2">
                                            @foreach ($Principal->departamento as $departamento)
                                                <li
                                                    class="list-group-item border-0 d-flex align-items-center gap-1 bg-transparent ">
            ...
            @role('Administrador')
                <div class="col-12 d-flex justify-content-end mt-3 gap-2">
                    <div>
                        <button class="btn fst-normal px-4 animated-icon button-eliminar" id="BtnP"
                               data-worker="{{ $id }}" data-id="{{ $Principal->id }}"  data-boton-presionado="BtnS" data-bs-toggle="tooltip"
                            data-bs-html="true" data-bs-title="Eliminar el nombramiento <b>principal</b> de la persona">
                    ...
                    <div>
                        <abbr title="Editar los datos del nombramiento.">
                            <button type="button" class="btn fst-normal px-4 animated-icon button-edit"
                                data-bs-toggle="modal" data-bs-target="#Editprincipal">
...
@else
    <div class="row cont-principal">
        <h5 class="text-center">Sin nombramientos</h5>
        <div class="col d-flex justify-content-center cont_img mb-2">
            <img src="{{ asset('images/no_data.webp') }}" alt="No hay nombramiento principal">
        </div>

        @role('Administrador')
            <div class="d-flex justify-content-center bg-color-form mt-0 p-0">
                <abbr title="Agregar nombramiento principal.">
                    <button class="btn fst-normal px-4 animated-icon button-add" id="N-Principal">
